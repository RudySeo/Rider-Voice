...
- 카카오 로그인 활성 사용자는 기본 `USER`로 생성하고, 공용 6자리 인증번호를 통과한 `RIDER`와 `ADMIN`만 리뷰를 작성·수정할 수 있게 한다.
...
- 자유 의견은 작성·수정 즉시 공개하고 신고·사후 조치와 음식점 정보 정정을 지원한다.
- 리뷰 작성 권한 사용자는 음식점별 활성 리뷰를 하나만 작성할 수 있게 한다.
- 활성 리뷰가 삭제되거나 전체 제외된 경우 최초 제출 시각부터 90일 후 다시 작성할 수 있게 한다.

## 제품 신뢰 경계

- CRITICAL: 카카오 로그인은 계정 식별 수단이며 `RIDER`는 공용 인증번호로 부여하는 서비스 내부 작성 권한이다.
- CRITICAL: 리뷰와 집계를 사실 보증 또는 조작 방지 보장으로 표현하지 않는다.
- CRITICAL: 공개 리뷰·리포트 응답에 방문 인증 상태나 `verificationStatus`·`verificationNotice`를 추가하지 않는다.
- CRITICAL: 서로 다른 작성자 5명 기준은 공개 집계 기준일 뿐 조작 방지 보장이 아니다.
...
- 구현 전에 `docs/PRD.md`, `docs/ARCHITECTURE.md`, `docs/ADR.md`, `docs/ERD.md`를 읽는다.
...
- 로그인 화면에서 카카오 로그인은 계정 식별이며 리뷰 작성에는 별도 `RIDER` 권한이 필요하다고 안내하고 OAuth 교환 시 신규 사용자를 `ACTIVE`, `USER` 상태로 생성한 뒤 opaque access/refresh token을 발급한다.
...
- `UserRole`은 `USER`, `RIDER`, `ADMIN`이며 access token 인증 시 현재 DB role을 확인한다.
- 공용 인증번호는 6자리 숫자이고 BCrypt hash만 DB에 저장한다. `ADMIN`은 전용 API로 언제든 현재 번호를 교체하며 기존 `RIDER` 권한은 유지한다.
- `USER`의 인증번호 입력은 계정별 5회 실패 시 15분 잠그고 DB에 실패·잠금 상태를 저장한다.

## 음식점 규칙

- `PickupLocation`은 실제 픽업 장소, `Restaurant`은 소비자에게 보이는 배달 브랜드다.
- 하나의 픽업 장소에 여러 배달 브랜드를 연결할 수 있다.
- 카카오 장소 ID는 `Restaurant.kakaoPlaceId`의 nullable unique 컬럼으로 관리한다.
...
- 음식점 사후 병합은 제공하지 않고 등록 단계의 장소·브랜드명 및 카카오 장소 ID unique 제약으로 중복을 줄인다.

## 리뷰 규칙

- CRITICAL: `RIDER`와 `ADMIN`만 리뷰를 생성·수정할 수 있고 application service가 현재 DB role을 다시 확인한다.
- `USER`, `RIDER`, `ADMIN`은 본인 리뷰 조회·삭제와 신고를 사용할 수 있다.
...
- 구조화 평가와 자유 의견은 작성·수정 즉시 공개한다.
- 공개 의견이 신고된 상태에서 수정되더라도 신고 처리 전까지 숨김을 유지한다.
- 신고 접수 시 공개 의견만 임시 숨기고 구조화 평가는 유지한다.
- 신고 기각은 의견 상태를 복원한다.
- 의견 위반은 의견만 비공개 처리한다.
- 허위·도배로 인정된 리뷰는 전체를 공개와 집계에서 제외한다.
- 관리자 결정과 음식점 정정은 `ModerationAudit`에 기록한다.
...
- 공개·USER·RIDER·ADMIN 인증 요구사항을 endpoint test로 검증한다.
- 공개 리뷰와 리포트 DTO에는 방문 인증 상태와 안내를 포함하지 않는다.
...
- 활성 리뷰 중복 작성, 삭제·전체 제외 후 90일 우회와 의견 신고 처리를 검증한다.
...
- 공개 API·UI에서 공용 인증번호를 사실 보증으로 표현하거나 방문 인증 상태를 다시 추가하거나 같은 픽업 장소의 다른 브랜드를 노출하는 변경을 지적한다. 역할 기반 작성 범위와 공개 범위를 유지하는 방향을 제안한다.
...
- 음식점별 활성 리뷰 하나, 삭제·전체 제외 후 최초 제출 기준 90일, 삭제·제외 기록을 포함한 24시간 제한, 신고된 의견 숨김, 작성자 5명 집계와 `NOT_OBSERVED` 분모 규칙을 우회하는 변경을 지적하고 해당 경계 테스트를 요구한다.
...
3. 공용 인증번호 기반 `RIDER` 역할과 리뷰 작성 권한
4. 픽업 장소·배달 브랜드·외부 참조 모델
5. 음식점별 활성 리뷰 1개, 삭제·전체 제외 후 90일 제한과 의견 신고 처리
6. 공개 검색·상세·리뷰와 작성자 5명 집계
7. 신고와 관리자 정정 처리
8. OpenAPI, 보안, 동시성과 로컬 MySQL 회귀 검증

추가 클라이언트와 배포 인프라는 실제 운영 필요성을 확인한 뒤 문서를 먼저 변경하고 착수한다.
