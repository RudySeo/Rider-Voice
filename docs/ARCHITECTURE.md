...
아키텍처는 서비스의 각 부분이 어떤 일을 맡고 어떻게 연결되는지를 뜻합니다. Rider Voice는 모바일 앱, API 서버, 데이터베이스와 카카오 서비스로 구성됩니다.
...
API 서버
  ...
  └── 공개·집계·신고 처리
       │
       ├── MySQL: 사용자, 음식점, 리뷰와 처리 기록 저장
       └── 카카오: 로그인과 장소·주소 검색
```

가장 중요한 원칙은 **모바일 앱이 데이터베이스나 카카오 API를 직접 호출하지 않는 것**입니다. 서버가 사용자 입력을 다시 확인하고, 리뷰 제한과 공개 기준을 한곳에서 적용합니다.

### 기능별 역할

| 영역 | 쉬운 설명 |
| --- | --- |
| 인증 | 카카오 계정을 Rider Voice 사용자와 연결하고 로그인 상태를 관리합니다. |
| 음식점 | 실제 픽업 장소와 소비자에게 보이는 배달 브랜드를 구분해 관리합니다. |
| 리뷰 | 작성·수정·삭제, 90일 제한과 항목별 집계를 처리합니다. |
| 관리 | 리뷰·음식점 신고를 접수하고 관리자 결정과 변경 기록을 남깁니다. |
| 공통 기반 | 오류 형식, 보안, 시간, 데이터 ID와 호출 제한을 관리합니다. |

### 자주 나오는 기술 용어

| 용어 | 이 프로젝트에서의 의미 |
| --- | --- |
| API | 모바일 앱이 서버에 검색이나 저장을 요청하는 통로입니다. |
| OAuth | 카카오 계정으로 로그인하기 위한 표준 절차입니다. |
| access token | 로그인 후 API를 호출할 때 사용하는 15분 유효 임시 키입니다. |
| refresh token | access token을 다시 발급받기 위한 30일 유효 키입니다. |
| 트랜잭션 | 여러 저장 작업을 모두 성공시키거나 모두 취소하는 처리 단위입니다. |
| 캐시 | 반복 검색을 빠르게 처리하기 위해 결과를 잠시 보관하는 공간입니다. |
| migration | 운영 데이터베이스 구조를 순서대로 안전하게 변경하는 기록입니다. |

이후 장은 개발과 운영에 필요한 상세 구조를 설명합니다. 제품 기능만 확인하려면 [PRD.md](PRD.md)를 먼저 읽어도 됩니다.
...
이 장은 개발·운영 담당자를 위한 기술 참고입니다. 일반 독자는 “모바일 앱이 API 서버를 거쳐 MySQL과 카카오를 사용한다”는 구조만 이해하면 충분합니다.
...
```text
mobile
├── src/app      # Expo Router 화면과 route
├── src/shared   # API client, 인증, 디자인 토큰과 공통 UI
├── src/components # Expo 기본 UI 구성 요소
└── assets       # 앱에 포함하는 폰트와 정적 자산
```

모바일 앱도 DB나 카카오 API를 직접 호출하지 않고 Spring Boot `/api/v1`만 호출한다. API 환경은 기본 `local`과 명시적 `aws` 프로필로 나누며 검색과 인증이 같은 resolved base URL을 사용한다. `local`은 simulator·emulator 주소 또는 개발 중인 Metro의 private LAN host를 사용하고 `aws`는 `.env.local`의 HTTPS 주소가 필수다. 공개 검색은 mock fallback 없이 선택된 API 서버가 반환한 내부 음식점과 카카오 결과를 `리뷰 있음`, `등록됨·리뷰 없음`, `카카오 장소`로 구분하고 사용자 좌표, 거리와 가까운 순을 요청하거나 추정하지 않는다. API 주소가 없거나 연결할 수 없으면 고정 결과로 대체하지 않고 설정·연결 오류를 표시한다. 선택 대상과 원 검색어를 리뷰 작성까지 보존한다. 모바일 API 타입은 OpenAPI에서 생성하며 생성 파일을 직접 수정하지 않는다.
...
검색 결과 조립은 후보 수에 비례한 DB 호출을 만들지 않는다. 내부 음식점의 현재 유효 작성자 수는 음식점 ID 집합으로 한 번에 조회하고, 카카오 후보와 연결된 음식점도 카카오 장소 ID 집합으로 한 번에 조회한다. 연결된 폐업 음식점은 외부 후보로 다시 노출하지 않으며, 미등록 후보만 카카오 장소로 반환한다.
...
초기에는 별도 집계 테이블이나 비동기 집계 작업을 만들지 않고 조회 쿼리와 application service에서 계산한다. 검색 목록의 작성자 수는 음식점 ID 집합을 사용한 배치 조회로 가져오고, 상세 리포트는 요청한 브랜드 또는 장소의 유효 리뷰를 조회해 계산한다. 삭제나 전체 제외로 작성자가 5명 미만이 되면 결과는 다시 `COLLECTING`으로 바뀐다.
...
실제 DB·카카오 secret은 Docker build와 이미지에 포함하지 않는다. CI는 일회용 DB 값과 dummy provider 값을 사용한다. Docker Hub PAT는 GitHub Environment secret으로 전달하고 운영 secret은 아래 운영 배포 경계에 정의한 SSM Parameter Store에서만 읽는다.
...
master push
  ...
  -> SSM Run Command
  -> 정확한 master commit의 release script와 Nginx 설정 다운로드
  -> EC2의 deploy script가 새 API container 기동과 health check

개발자 PC
  -> 로컬 Spring API 127.0.0.1:8080
  -> 로컬 Prometheus와 Grafana Docker Compose
```
...
- 새 container는 commit 기반 불변 태그로 실행하고 `/actuator/health`가 제한 시간 안에 `UP`이 아니면 이전 image를 다시 실행한다. Flyway schema는 자동 rollback하지 않으므로 변경은 이전 application·data와 호환되게 유지한다.
...
- 운영 문제는 API health, application log와 SSM 진단으로 확인한다. 외부 장애 감지나 장기 metric 보관이 필요해지면 별도 host 또는 관리형 관측 서비스를 새로 결정한다.
