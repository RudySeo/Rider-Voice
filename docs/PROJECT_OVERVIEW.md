...
중복 음식점은 hard delete하지 않고 `MERGED` 상태와 canonical 음식점 ID를 유지한다. 기존 ID로 들어온 조회와 작성 요청도 canonical 음식점으로 해석한다.
...
- 같은 음식점에는 마지막 제출 후 90일마다 새 리뷰를 작성할 수 있다.
- 새 리뷰가 등록되면 이전 리뷰는 공개 이력으로 남지만 더 이상 수정할 수 없다.
- 현재 최신 리뷰만 수정하거나 삭제할 수 있고 방문 연월은 수정할 수 없다.
- 최신 리뷰를 삭제하거나 관리자가 전체 제외해도 마지막 제출 시각과 순번을 유지한다.
- 삭제·제외 후 과거 리뷰를 현재 집계 대상으로 복원하지 않는다.
- 타인의 리뷰와 수정할 수 없는 과거 리뷰는 `404 Not Found`로 처리한다.

리뷰 이력은 `Review`, 작성자와 음식점별 현재 상태는 `AuthorRestaurantReviewState`로 분리한다. 상태 row locking과 데이터베이스 unique 제약으로 90일 정책과 동시 요청을 직렬화한다.

## 6. 공개 집계

개별 구조화 리뷰는 유효하면 첫 작성부터 공개한다. 집계는 브랜드와 픽업 장소를 독립적으로 계산한다.

- 유효 현재 작성자 0명: `NO_REVIEWS`
- 서로 다른 유효 현재 작성자 1~4명: `COLLECTING`
- 서로 다른 유효 현재 작성자 5명 이상: `PUBLISHED`

브랜드 집계는 작성자별 해당 브랜드의 현재 리뷰 하나를 사용한다. 장소 집계는 같은 작성자가 한 장소의 여러 브랜드에 현재 리뷰를 가지고 있더라도 생성 시각과 ID가 가장 최신인 하나만 반영한다.
...
- 관리자 API: 의견 검수, 신고 처리, 픽업 장소 정정과 음식점 병합
...
- Kotlin 2.3, JDK 21, Gradle Kotlin DSL
