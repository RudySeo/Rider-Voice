...
**선택**: 실제로 음식을 받는 장소는 `PickupLocation`, 소비자와 배달 앱에 보이는 브랜드는 `Restaurant`으로 나눈다. 한 픽업 장소에는 여러 브랜드를 연결할 수 있고 카카오 장소 ID는 별도 외부 참조로 관리한다. 브랜드명은 저장 전에 NFKC와 공백을 정리하며, 별도 정규화 이름 컬럼 없이 MySQL `utf8mb4_0900_ai_ci`의 대소문자 비구분 비교로 같은 장소의 중복을 막는다.
