...
**감수할 점**: 브라우저와 모바일의 인증 전달 방식이 달라 모바일용 서버 계약과 보안 테스트가 추가로 필요하다. Expo Web에서 확인한 화면만으로 네이티브 동작을 보장할 수 없으므로 iOS Simulator와 Android Emulator에서 별도 검증해야 한다.
...
앱은 access token을 메모리에만 두고 refresh token을 SecureStore에 저장한다. refresh는 매번 token을 회전하며 로그아웃은 server session을 폐기하고 로컬 token을 항상 제거한다. Expo Go와 Expo Web은 고정 custom scheme을 신뢰할 수 있는 실제 인증 환경으로 취급하지 않고 공개 mock 조회만 제공한다.
