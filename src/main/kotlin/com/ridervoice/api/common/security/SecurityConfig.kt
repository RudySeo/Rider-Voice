...
class SecurityConfig(
    ...
    fun apiSecurityFilterChain(http: HttpSecurity): SecurityFilterChain = http
        ...
        .authorizeHttpRequests {
            ...
            it.requestMatchers(HttpMethod.POST, "/api/v1/reviews").hasAnyRole("RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.PATCH, "/api/v1/reviews/*").hasAnyRole("RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.GET, "/api/v1/addresses/search").hasAnyRole("RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.POST, "/api/v1/users/me/rider-verification").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.POST, "/api/v1/reviews/*/reports").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.POST, "/api/v1/restaurants/*/reports").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.DELETE, "/api/v1/reviews/*").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.GET, "/api/v1/users/me").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.GET, "/api/v1/users/me/reviews").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.GET, "/api/v1/reviews/*").hasAnyRole("USER", "RIDER", "ADMIN")
            it.requestMatchers(HttpMethod.PUT, "/api/v1/admin/rider-invite-code").hasRole("ADMIN")
