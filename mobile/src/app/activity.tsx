  ...
  return (
    <Screen footer={<BottomTabBar active="activity" />}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {auth.user.role === 'USER' && <RiderVerificationCard onVerify={auth.verifyRider} />}
        ...
        {writer && <View style={styles.action}><PrimaryButton label="새 리뷰 작성" onPress={() => router.push('/')} tone="outline" /></View>}
...
function RiderVerificationCard({ onVerify }: { onVerify: (code: string) => Promise<unknown> }) {
  const [code, setCode] = useState('');
  const [loading, setLoading] = useState(false);
  const submit = async () => {
    if (!/^[0-9]{6}$/.test(code)) {
      Alert.alert('인증번호를 확인해주세요', '숫자 6자리를 입력해주세요.');
      return;
    }
    try {
      setLoading(true);
      await onVerify(code);
      setCode('');
      Alert.alert('라이더 권한이 활성화됐어요', '이제 리뷰를 작성하고 수정할 수 있어요.');
    } catch (error) {
      Alert.alert('인증하지 못했어요', riderVerificationErrorMessage(error));
    } finally { setLoading(false); }
  };
  return <View style={styles.verifyCard}><AppText variant="section">라이더 권한 인증</AppText><AppText color={colors.muted} variant="caption">운영자에게 받은 6자리 인증번호를 입력해주세요.</AppText><TextInput accessibilityLabel="라이더 인증번호" keyboardType="number-pad" maxLength={6} onChangeText={(value) => setCode(value.replace(/[^0-9]/g, ''))} placeholder="숫자 6자리" placeholderTextColor={colors.muted} secureTextEntry style={styles.codeInput} value={code} /><PrimaryButton disabled={code.length !== 6} label="인증하기" loading={loading} onPress={submit} /></View>;
}
...
const styles = StyleSheet.create({
  ...
  verifyCard: { gap: spacing.sm, padding: spacing.md, marginBottom: spacing.lg, backgroundColor: colors.jadeSoft, borderRadius: radius.md }, codeInput: { minHeight: 48, paddingHorizontal: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.surface, color: colors.text, letterSpacing: 8, fontSize: 18 },
});
