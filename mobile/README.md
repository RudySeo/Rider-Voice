...
Rider Voice의 React Native 앱입니다. Expo SDK 57과 Expo Router를 사용하며 iOS, Android, Web 미리보기를 지원합니다.
...
실행 후 터미널에서 `i`, `a`, `w`를 눌러 각각 iOS, Android, Web을 열 수 있습니다.
...
- 모바일 카카오 로그인: 백엔드가 일회용 인증 코드를 앱 딥링크로 전달하고 앱이 토큰으로 교환하는 계약이 필요합니다.
- 상세 헬멧 점수: 클라이언트가 분포로 점수를 임의 계산하지 않도록 백엔드 응답에 항목별 `score`가 필요합니다.

이 두 기능은 UI를 미리 구현했지만, 실제 인증·실데이터 동작은 서버 계약을 추가한 뒤 연결합니다.
